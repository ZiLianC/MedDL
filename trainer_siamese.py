...
def train_epoch(model,
    ...
    for idx, batch_data in enumerate(loader):
        ...
        data1,data2, target = data1.cuda(args.rank),data2.cuda(args.rank), target.cuda(args.rank)
        ...
        logits1 = model(data1)
        logits2 = model(data2)
        output = bottom_net((logits1 - logits2) ** 2)
...
def val_epoch(model,
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(loader):
            ...
            _,result=torch.max(logits,1)
            y_pred.append(result.cpu().item())
